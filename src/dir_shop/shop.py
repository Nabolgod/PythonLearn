...
class Shop:
    ...
    def get_items(self, category_search, name_item_search):
        """Метод возвращает список всех товаров определённой категории и названия"""
    ...
    def remove_item(self, category, name_item, id_item):
        "Метод для удаления товара из хранилища магазина"
        items = self.get_items(category, name_item)
        for item in items:
            if item.id == id_item:
                items.remove(item)
                break
    ...
    def items(self, category_search):
        return self.storage.get(category_search).keys()
